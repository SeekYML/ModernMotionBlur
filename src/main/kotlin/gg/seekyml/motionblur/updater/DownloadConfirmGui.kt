...
import gg.seekyml.motionblur.updater.Updater.shouldUpdate
import gg.seekyml.motionblur.updater.Updater.updateUrl
...
class DownloadConfirmGui(private val parent: GuiScreen?) : GuiScreen() {
        ...
        when (button.id) {
            0 -> {
                ...
                Multithreading.runAsync {
                    if (Updater.download(
                            updateUrl,
                            File("mods/${MotionBlur.NAME}-${Updater.latestTag.substringAfter("v")}.jar")
                        ) && Updater.download(
                            "https://github.com/Wyvest/Deleter/releases/download/v1.2/Deleter-1.2.jar",
                            File(MotionBlur.modDir.parentFile, "Deleter-1.2.jar")
                        )
                    ) {
                        EssentialAPI.getNotifications()
                            .push(MotionBlur.NAME, "The ingame updater has successfully installed the newest version.")
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        drawCenteredString(
            fontRendererObj,
            EnumChatFormatting.DARK_PURPLE.toString() + MotionBlur.NAME,
            width / 4,
            3,
            -1
        )
        ...
        val lines = listOf(
            ...
            "(This will update from v${MotionBlur.VERSION} to ${Updater.latestTag})"
        )
